...
public class DatabaseHelper {
	...
	public String getScriptures(Context context, String category) {
		Cursor cursor;
		if (category != null)
			cursor = db.rawQuery("SELECT * FROM scriptures WHERE category=?", new String[] {category});
		else
			cursor = db.rawQuery("SELECT * FROM scriptures", null);
		...
		while (cursor.moveToNext()) {
			...
			try {
				...
				result.put(cursor.getPosition(), jsonObject);
	...
	public boolean insertScriptures(String jsonData) {
		String insertSql = "INSERT INTO scriptures(scripture, category, timestamp) values (?, ?, ?)";
		JSONArray jsonArray;
		try {
	...
	private static class OpenHelper extends SQLiteOpenHelper {		
		...
		public OpenHelper(Context context) {
			super(context, "pbbible.db", null, 6);
		}
