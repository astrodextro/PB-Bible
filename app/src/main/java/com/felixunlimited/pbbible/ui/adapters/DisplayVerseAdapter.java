...
//				txtVerse.setBackgroundColor(Color.TRANSPARENT);
	...
	private Spannable parseVerseForDisplay(DisplayVerse displayVerse, View v) {
		...
		for (int i=0; i < posVerseList.size(); i++) {
			...
			spanText.setSpan(new ForegroundColorSpan(Color.MAGENTA), posVerse, posVerse+strI.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
		}
		for (int i=0; i<posTSBeginList.size(); i++) {
			int posTSBegin = posTSBeginList.get(i);
			int posTSEnd = posTSEndList.get(i);
			spanText.setSpan(new ForegroundColorSpan(Color.rgb(250,200,50)), posTSBegin, posTSEnd, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
		}
		...
		for (int i=0; i<posRFList.size(); i++) {
			...
			spanText.setSpan(new ClickableSpan() {
				...
				public void onClick(View widget) {
					BiblesOfflineActivity mainActivity = (BiblesOfflineActivity) context;
			...
			spanText.setSpan(new ForegroundColorSpan(Color.MAGENTA), posRF, posRF+2, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
		}
